...
SETUP_HELP = {
    "missing": (
        ...
        "          | Select-Object -First 1).FullName auth login\n\n"
        "Then start Claude Vitals again. You don't have to use Claude Code\n"
        "afterwards — that's just where the login gets stored.\n\n"
        "Note: 'claude setup-token' is NOT the same thing. It prints a\n"
        "long-lived token for use as an environment variable and never writes\n"
        "the credentials file, so Claude Vitals still won't find a login.\n\n"
